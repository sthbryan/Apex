...
import { countdown, pacing, resetIn, resetText, roughly } from "./format";
...
describe("pacing", () => {
  ...
  it("returns onPace when lasts_to_reset is true", () => {
    expect(pacing({ lasts_to_reset: true } as never)).toEqual({ text: "on pace", tone: "text-state-done" });
  });
  ...
  it("returns overPace when eta is null", () => {
    expect(pacing({ lasts_to_reset: false, eta_seconds: null } as never)).toEqual({ text: "over pace", tone: "text-state-blocked" });
  });
});

describe("countdown edge", () => {
  it("handles exactly one day", () => {
    expect(countdown(86400)).toBe("1d0h");
  });
});

describe("roughly edge", () => {
