...
export const OPTIONAL_GROUPS: ToolGroup[] = ["observation", "orchestration", "views", "browser"];

export function toolsOff(agent: string): ToolGroup[] {
  return agents.value.find((found) => found.name === agent)?.tools_off ?? [];
}

export function groupOn(agent: string, group: ToolGroup): boolean {
  return !toolsOff(agent).includes(group);
}

export async function setGroupOn(agent: string, group: ToolGroup, on: boolean): Promise<void> {
